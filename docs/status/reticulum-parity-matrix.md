...
| `RNS/Link.py` | `crates/libs/rns-transport` | partial | unit, pinned-python | Establishment, proof validation, bound-interface enforcement for data/channel fan-out, RTT-derived liveness, protocol close, cleanup, and single-packet requests and responses (`request_packet`/`response_packet`, the send halves of the `PacketContext::Request`/`Response` handling that already existed on receive — matching `request`/`handle_request`'s packet-or-resource choice). | RNS 1.4.2 link-proof path rebalancing remains unimplemented. Choosing between a packet and a resource is left to the caller in both directions; the crate does not apply the `<= self.mdu` test on its behalf. A packet-borne request also carries no id of its own — the responder derives it from the packet hash — so a requester must correlate on `packet.hash()` rather than an id it chose. |
| `RNS/Resource.py` | `crates/libs/rns-transport` | partial | unit, simulated, pinned-python | Bounded receive allocation, advertisement validation, retries, window-local hashmap exhaustion gated on an outstanding update (matching `request_next`/`waiting_for_hmu`, with a bounded wait so a lost update cannot park the transfer), adaptive fragment scheduling on Python's own ladder (`WINDOW`/`WINDOW_MIN`/`WINDOW_MAX_SLOW`/`WINDOW_MAX_VERY_SLOW`/`WINDOW_MAX_FAST`/`WINDOW_FLEXIBILITY` with rate-measured ceiling changes), timeout/failure events, cancellation, cleanup, and Python-shaped split-resource sequencing with original-hash preservation, ordered reassembly, per-segment completion metadata, and final whole-resource completion. | The sender has no `receiver_min_consecutive_height` serving window (`COLLISION_GUARD_SIZE`), so a Rust↔Rust transfer cannot exercise the behaviour a Rust↔Python one does — receiver-side parity has to be evidenced against Python. |
