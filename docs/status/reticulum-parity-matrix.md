...
- UDP unicast and multicast with peer routing, multicast proof fallback,
  ...
  wildcard TCP server listeners, including `localhost` and `0.0.0.0`, plus
  explicit UDP listener, peer, and multicast-bind records through
  `set_interfaces` and `reload_config`, while `device`-bound, non-local
  concrete, and broader TCP server listener shapes, plus UDP
