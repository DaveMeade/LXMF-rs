...
| `RNS/Link.py` | `crates/libs/rns-transport` | partial | unit, pinned-python | Establishment, proof validation, bound-interface enforcement for data/channel fan-out, RTT-derived liveness, protocol close, cleanup, and single-packet request responses (`response_packet`, the send half of the `PacketContext::Response` handling that already existed on receive — matching `handle_request`'s packet-or-resource choice). | RNS 1.4.2 link-proof path rebalancing remains unimplemented. Choosing between a response packet and a response resource is left to the caller; the crate does not apply `handle_request`'s `len(packed_response) <= self.mdu` test on its behalf. |
| `RNS/Resource.py` | `crates/libs/rns-transport` | partial | unit, simulated, pinned-python | Bounded receive allocation, advertisement validation, retries, window-local hashmap exhaustion gated on an outstanding update (matching `request_next`/`waiting_for_hmu`, with a bounded wait so a lost update cannot park the transfer), outbound bz2 auto-compression matching `Resource.__init__`'s `auto_compress` default, timeout/failure events, cancellation, cleanup, and Python-shaped split-resource sequencing with original-hash preservation, ordered reassembly, per-segment completion metadata, and final whole-resource completion. | Fragment scheduling is **not** adaptive: `WINDOW` is a fixed 4 where Python grows it 4→10→75 (`WINDOW_MIN`/`WINDOW_MAX_SLOW`/`WINDOW_MAX_FAST`/`WINDOW_FLEXIBILITY`, `Resource.py`), so throughput against a real sender is well below parity even though transfers now complete. The sender also has no `receiver_min_consecutive_height` serving window (`COLLISION_GUARD_SIZE`), so a Rust↔Rust transfer cannot exercise the behaviour a Rust↔Python one does. |
