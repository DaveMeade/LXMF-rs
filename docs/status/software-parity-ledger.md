...
client-specific compatibility claims are tracked separately and must not be
...
- Sideband, MeshChatX, or other external-client release evidence. Columba
  release evidence is tracked in the LXMF matrix and roadmap, not as software
  ledger row-closing evidence.
...
| Sideband, MeshChatX, and other external clients | `LXMF/LXMRouter.py`, `LXMF/Handlers.py` | Client-specific compatibility claims require separate release evidence; Columba now has local bidirectional direct LXMF evidence from `tools/scripts/external-client-interop-gate.sh columba /home/pgiuseppe/Documents/columba` against clean Columba `3738bd7834128023db491cca0876585b799de942` (`v2.0.9-beta`). |
