...
- `Link::response_packet` completes the request/response pair: the receive
  half already decrypted `PacketContext::Response`, but nothing could build
  one, so a responder had to send every reply as a resource transfer even
  when the packed response fits a single packet. Python chooses per reply
  (`Link.handle_request`); the choice is the caller's here, and the crate now
  exposes both options.
