...
2. For this candidate, tag the reviewed commit: `git tag -a v0.9.9-rc.2 -m "LXMF-rs v0.9.9-rc.2" && git push origin v0.9.9-rc.2`.
...
# 3. Verify build provenance of any file:
  gh attestation verify lxmf-rs_0.9.9-rc.2_linux-x86_64.tar.gz --owner FreeTAKTeam

# 4. Verify the container image:
  cosign verify ghcr.io/freetakteam/lxmf-rs:0.9.9-rc.2 \
  --certificate-identity-regexp "https://github.com/FreeTAKTeam/LXMF-rs/.*" \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
  gh attestation verify oci://ghcr.io/freetakteam/lxmf-rs:0.9.9-rc.2 --owner FreeTAKTeam
```
