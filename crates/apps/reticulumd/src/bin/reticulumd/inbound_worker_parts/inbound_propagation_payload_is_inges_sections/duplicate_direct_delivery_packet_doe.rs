    ...
    #[tokio::test]
    async fn direct_delivery_resource_above_limit_emits_drop_event_without_side_effects() {
        let daemon = RpcDaemon::test_instance();
        daemon
            ...
            .expect("set delivery resource limit");
        let delivery_private = PrivateIdentity::new_from_rand(OsRng);
        ...
        let transport_identity = to_transport_private_identity(&delivery_core_private);
        let transport = Transport::new(TransportConfig::new("test", &transport_identity, true));
        let oversized_content = "x".repeat(1200);
        let wire = build_wire_message_with_options(
            source_hash,
            destination_hash,
            "oversized direct resource",
        ...
        .expect("wire");
        assert!(wire.len() > 1000, "test resource must exceed one delivery-limit KB");
        ...
        let event = daemon.take_event().expect("drop event");
        assert_eq!(event.event_type, "inbound_dropped");
        assert_eq!(event.payload["reason"], json!("delivery_resource_too_large"));
        assert_eq!(event.payload["delivery_kind"], json!("resource"));
        let raw_destination = hex::encode(destination_hash);
        assert!(event.payload["raw_destination_hash"]
            .as_str()
        ...
        assert!(
            event
                .payload["detail"]
                .as_str()
                .is_some_and(|detail| detail.contains("delivery limit 1000 bytes")),
            "drop event should include bounded resource-limit diagnostics: {:?}",
            event.payload
        );
        assert!(daemon.take_event().is_none(), "oversized resource should emit one event");

        let messages = daemon
            .handle_rpc(RpcRequest { id: 53, method: "list_messages".to_string(), params: None })
            .expect("list messages")
            .result
            .expect("list messages result");
        assert_eq!(messages["messages"].as_array().expect("message items").len(), 0);
