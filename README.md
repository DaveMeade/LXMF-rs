...
enterprise quality gates. The `0.9.0` line is a usable sub-1.0
...
Current release train: `0.9.0`.

Use `docs/release-notes-v0.9.0.md` for the release summary and
...
The `0.9.0` release scope covers the Rust libraries, SDK entry points, `lxmd`,
`reticulumd`, and `rns-tools`, plus host-native GitHub bundles for all
implemented user-facing tools. The `0.9.0` train is SDK-first: it improves the
...
cargo xtask package-daemon-bundle --version 0.9.0
...
make package-daemon-bundle VERSION=0.9.0
...
For `v0.9.0`, use the release at:

[https://github.com/FreeTAKTeam/LXMF-rs/releases/tag/v0.9.0](https://github.com/FreeTAKTeam/LXMF-rs/releases/tag/v0.9.0)
...
```bash
sha256sum -c lxmf-rs-tools-v0.9.0-linux-x64.tar.gz.sha256
tar -xzf lxmf-rs-tools-v0.9.0-linux-x64.tar.gz

sha256sum -c lxmf-rs-tools-v0.9.0-macos-arm64.tar.gz.sha256
tar -xzf lxmf-rs-tools-v0.9.0-macos-arm64.tar.gz
```

3. Windows

```powershell
Get-FileHash .\lxmf-rs-tools-v0.9.0-windows-x64.zip -Algorithm SHA256
Get-Content .\lxmf-rs-tools-v0.9.0-windows-x64.zip.sha256
Expand-Archive .\lxmf-rs-tools-v0.9.0-windows-x64.zip .
```
